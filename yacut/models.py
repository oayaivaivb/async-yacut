...
class URLMap(db.Model):
    ...
    short = db.Column(db.String(MAX_SHORT_ID_LENGTH), unique=True, nullable=False)
...
class File(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    original_name = db.Column(db.String(MAX_ORIGINAL_NAME_LENGTH), nullable=False)
    short = db.Column(db.String(MAX_SHORT_ID_LENGTH), unique=True, nullable=False)
    yandexdisk_path = db.Column(db.String(MAX_PATH_LENGTH), nullable=False)
    timestamp = db.Column(db.DateTime, index=True, default=datetime.now(UTC))